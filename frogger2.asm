...
lw $s0, car1_x		# load the position of car1 
add $a2, $s0, $k0 	# set the x location of the first car  $a2 = og position ($s0) + increment ($k0)
addi $s1, $zero, 32 	# $s1 == 32 (so that we can divide and wrap around the positions)
div $a2, $s1		# Divide $a2 by $s1  
mfhi $a2 		# Get the remainder of the division and assign to $a2 (mod) 

lw $s0, car2_x		# load the position of car2
add $a3, $s0, $k0  	# set the x location of the second car $a3 = og position ($s0) + increment ($k0)
addi $s1, $zero, 32 	# $s1 == 32 (so that we can divide and wrap around the positions)
div $a3, $s1		# wrap around the position 
mfhi $a3 		# Get the remainder of the division and assign to $a3 (mod)
...
allocate_memory:
	...
	# If $t5 is greater than 32, then overflow control 
	# Overflow_control:
	# If a2 (or a3) is greater than 32 - (size of block), keep track of how much is overflowing 
	# Run a loop that amount of times colouring in the first few pixels yellow 
...
addi $k0, $k0, 1 	# Increment for repaint 
